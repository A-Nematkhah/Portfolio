...
export function ProjectEditor({ initial, defaultCategory, onClose, onSaved }: Props) {
    ...
    try {
      for (let i = 0; i < arr.length; i++) {
        try {
          ...
          toast.error(`Failed to upload ${arr[i]?.name ?? "file"}: ${msg}`);
        }
      ...
      if (newMedia.length > 0) {
        ...
        toast.success(
          `Uploaded ${newMedia.length} file${newMedia.length === 1 ? "" : "s"}${failed ? ` (${failed} failed)` : ""}`,
        );
      } else if (failed > 0) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!payload.thumbnail_url && !payload.video_url) {
      setErr("Add at least one image or video, or set a video URL");
      setSaving(false);
      return;
    }
    if (import.meta.env.DEV) console.log("[ProjectEditor] saving payload", payload);
    const op = initial?.id
    ...
    if (import.meta.env.DEV) console.log("[ProjectEditor] saved row", data);
    ...
    >
      ...
      >
        <button
          ...
          aria-label="Close"
          data-modal-close
          className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full bg-background/80 hover:text-primary transition"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
                ...
                {media.map((m, i) => (
                  <div key={`${m.src}-${i}`} className="relative group">
                    {m.type === "image" ? (
                      ...
                      <div className="grid h-20 w-full place-items-center rounded-md bg-black/60 text-[10px] text-primary">
                        VIDEO
                      </div>
                    )}
                    <button
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button
              type="button"
              onClick={close}
              className="rounded-lg border border-border px-4 py-2 text-sm hover:border-primary/50 transition"
            >
              Cancel
            </button>
            <button
